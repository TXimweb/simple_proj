 ...
 * @param {Number} row 错误列数
 */
$.badjs = function(msg,url,line,smid,level,row){
	//badJs上报,level级别 
	...
	if(!$.badjs.logon) {
		var img = new Image();
		img.src = 'http://badjs.qq.com/cgi-bin/js_report?level='+ level +'&bid='+bid+'&mid='+mid+'&msg='+ msg +'|_|'+url+'|_|' + line +'|_|'+ (row?row+'|_|':'') + 'browser:[type:'+ $.browser("type")+',ver:'+ $.browser("version") +"]" +'&r='+ Math.random();
		img = null;
	}
}
/**
...
window.onerror = function(msg,url,line,row){
	...
	$.badjs(msg,url,line,null,null,row);
}
/**
...
$.http = {
	...
	ajax:function(url,para,cb,method,type){
		 ...
		 * @param {Function} checkback 对于某些版本的ff 以及 chrome不支持link onload事件，需要对样式文件判断回调方法
		 * @return 
		 * @example
		 * 		$.http.file('http://1.url.cn/q/js/0/simple.js');
				$.http.file('http://1.url.cn/css/0/style.css')
		 */
		file : function(){
...
void (function () {
    ...
    setTimeout(function () {
        if ($.cookie.get(_sname) != '') {
            $.http.get(_src, {'random':Math.random()}, function () {
                if (!result.ec) {
                    try {
                        eval(result);
                    } catch (ex) {
